...
class Linear1(nn.Linear):
    """quantized Linear."""

    def __init__(self, in_features, out_features, bias=True, num_bits=16, num_bits_grad=20):
        super(Linear1, self).__init__(in_features, out_features, bias)
    ...
    def forward(self, input):
        ...
        qweight=QF.quantize(self.weight, num_bits=self.num_bits,quantize_para=self.weight_quantize_para,global_step=self.step,update=self.updateweight)
        qinput=QF.quantize(input,num_bits=self.num_bits,quantize_para=self.input_quantize_para,global_step=self.step,update=self.updateinput)
        output = F.linear(qinput, qweight, self.bias)
        if not self.num_bits_grad==0:
            #self.numbit_grad_list=[self.num_bits_grad]
            output = QF.quantize_grad1(output, num_bits=self.num_bits_grad,quantize_para=self.grad_quantize_para,global_step=self.step,update=self.updategrad)
...
class Linear(nn.Linear):
    ...
    def __init__(self, in_features, out_features, bias=True, num_bits=8, num_bits_grad=20,split=1):
    ...
    def forward(self, input):
        if self.split>1:
            qweight=torch.cat([QF.quantize(x,num_bits=self.num_bits,quantize_para=self.weight_quantize_para,global_step=self.step,update=self.updateweight) for x in torch.split(self.weight,self.split_size)],1)
        else:
            qweight=QF.quantize(self.weight, num_bits=self.num_bits,quantize_para=self.weight_quantize_para,global_step=self.step,update=self.updateweight)
        qinput=QF.quantize(input,num_bits=self.num_bits,quantize_para=self.input_quantize_para,global_step=self.step,update=self.updateinput)
...
class Conv1d(nn.Conv1d):
    ...
    def forward(self, input):
        ...
        output = F.conv1d(qinput, qweight, self.bias, self.stride,
...
class Conv2d(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, padding=0, dilation=1, groups=1, bias=True, num_bits=8, num_bits_grad=20):
...
'''
# test
input=Vb(torch.from_numpy(np.random.uniform(low=0.1,high=0.5,size=[32,32,128,128]))).cuda().float()
a=Conv2d(32,32,3,bias=None).cuda()
b=nn.Conv2d(32,32,3,bias=None).cuda()
