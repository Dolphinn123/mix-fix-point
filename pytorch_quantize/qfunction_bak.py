...
class base_quantize(Function):
    ...
    def forward(cls,ctx, input, num_bits=8,inplace=False,quantize_para=torch.tensor([1,0,0],dtype=torch.float),global_step=None,update=None):
        ...
        #offset
        if config.offset:
            if not global_step==update.offset_updatestep:
            ...
            offset_update_flag=0
        #shift 
        if not global_step==update.shift_updatestep:
            step=2**shift
        ...
        #scale
        if config.scale:
             if not global_step==update.scale_updatestep:
                 A=A*scale
                 scale_update_flag=0
             else:
                 scale_update_flag=1
        ...
        if  not scale_update_flag or not shift_update_flag:
            output=torch.max(output,-A+offset)
    ...
    def backward(ctx, grad_output):
        ...
        if config.nbit_update=='mean_diff':
            if ctx.update.diffnum>config.update_th:
                ctx.num_bits[0]=ctx.num_bits[0]+8
        return grad_input, None, None,None,None,None
...
class update(object):
    def __init__(self,mode='forward',shift_updatestep=0,offset_updatestep=0,scale_updatestep=0):
        ...
        self.shift_mv=torch.tensor(100,dtype=torch.float).cuda()
        self.offset_mv=torch.tensor(100,dtype=torch.float).cuda()
        self.scale_mv=torch.tensor(100,dtype=torch.float).cuda()
    ...
    def updatefunction(self,mv,newnum,step):
        if step<config.step1:
            ...
            return config.alpha*newnum+(1-config.alpha)*mv,torch.min(torch.max(torch.round(config.beta/diffupdate-config.gama),torch.tensor(1,dtype=torch.float).cuda()),torch.tensor(454,dtype=torch.float).cuda())+step
        else:
