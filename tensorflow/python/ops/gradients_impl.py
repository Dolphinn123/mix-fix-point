...
from tensorflow.python.ops.bp_quantify import quantify #liuchang
...
def gradients(ys,
  ...
  with ops.get_default_graph()._lock:  # pylint: disable=protected-access
    return _GradientsHelper(ys, xs, global_step, mode, interval, steps_per_epoch, grad_ys, name, colocate_gradients_with_ops,#liuchang
                            gate_gradients, aggregation_method, stop_gradients,None)
...
def _GradientsHelper(ys,
          ...
          with ops.name_scope(op.name + "_grad"):
            ...
            with src_graph._original_op(op):
              ...
              if grad_fn:
                ...
                # functions.
                if op.type=="Conv2D" or op.type=="MatMul" or op.type=="BatchMatMul":
                  flag = flag + 1
                  print("Found conv or matmul:",op.name)
                  if isinstance(out_grads, list):
                    if len(out_grads) == 1:
                      out_grads = out_grads[0]
                    else:
                      print("out_grad has more than one tensor")
                    
                  out_grads,shift_op,bitnum_op,update_step_op,m_op = quantify(out_grads, global_step, mode=mode, interval=interval, steps_per_epoch=steps_per_epoch)
                  if mode == 3:
                    shift_list.append(shift_op)
                    bitnum_list.append(bitnum_op)
                    update_step_list.append(update_step_op)
                    m_list.append(m_op)
                  if mode == 2:
                    shift_list.append(shift_op)
                    bitnum_list.append(bitnum_op)
                  if mode == 1:
                    shift_list.append(shift_op)
                      ...
                      #end liuchang
                  if isinstance(out_grads, tf.Tensor):
                    out_grads = [out_grads]
                if op.type=="WinoConv":
                  top_diff_product_shift = tf.Variable(2.0, name='lc_top_diff_product_shift', trainable=False, dtype=tf.float32)
                  top_diff_product_scale = tf.Variable(1.0, name='lc_top_diff_product_scale', trainable=False, dtype=tf.float32)
                  top_diff_product_mean = tf.Variable(100.0, name='lc_top_diff_product_mean', trainable=False, dtype=tf.float32)
                 # out_grads[0], shift, bitnum, update_step, m = quantify(out_grads[0], global_step, mode=mode, interval=interval,steps_per_epoch=steps_per_epoch)
                  out_grads[3] = top_diff_product_shift
                  out_grads[6] = top_diff_product_scale
                  out_grads[9] = top_diff_product_mean

                in_grads = _MaybeCompile(grad_scope, op, func_call,
                                         lambda: grad_fn(op, *out_grads))
                if op.type=="WinoConv":
                  print("Found wino grad conv.")
                  flag = flag + 1
                  grad_shift_assign = tf.assign(top_diff_product_shift, in_grads[4])
                  grad_scale_assign = tf.assign(top_diff_product_scale, in_grads[7])
                  grad_mean_assign = tf.assign(top_diff_product_mean, in_grads[10])
                  with tf.control_dependencies([grad_shift_assign,grad_scale_assign,grad_mean_assign]):
                    in_grads[1] = tf.identity(in_grads[1])  
              else:
  ...
  return [_GetGrad(grads, x) for x in xs], shift_list, bitnum_list, update_step_list, m_list #liuchang
...
def hessians(ys,
  ...
  for gradient, x in zip(_gradients, xs):
    ...
    n = array_ops.size(x)
    loop_vars = [
        array_ops.constant(0, dtypes.float32),
