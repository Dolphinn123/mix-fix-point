...
from tensorflow.python.keras.layers.fp_quantify import fixed
from tensorflow.python.keras.layers.finetune_quantify import quantify
from tensorflow.python.keras.layers.inf_quantify import fixed_inf
...
class Conv(Layer):
  ...
  def call(self, inputs):
   ...
   # inputs, shift_inputs, f_inputs, offset_inputs = fixed_inf(inputs, 0, bitnum=8, mode=2, isfp16=isfp16)
   # self.kernel, shift_w, f_w = fixed_inf(self.kernel, 0, bitnum=8, mode=1, isfp16=isfp16)
    
   # self.kernel,_ = fixed(self.kernel,bitnum=8,interval=3,steps_per_epoch=1250)#liuchan
   # inputs,_ = fixed(inputs,bitnum=8,interval=3,steps_per_epoch=1250)
   # inputs,x_shift,x_bitnum,x_update_step,x_m = quantify(inputs, 'lc', steps_per_epoch=1250, th=3)
   # self.kernel,w_shift,w_bitnum,w_update_step,w_m = quantify(self.kernel, 'lc', steps_per_epoch=1250, th=6)
    outputs = self._convolution_op(inputs, self.kernel)
    if isfp16 is True:
      outputs = tf.cast(outputs, tf.float16)
      self.kernel = tf.cast(outputs, tf.float16)
      scale_inputs = tf.cast(tf.pow(2.0, - shift_inputs) * f_inputs, tf.float16) 
      scale_w = tf.cast(tf.pow(2.0, - shift_w) * f_w, tf.float16)
      offset_inputs = tf.cast(offset_inputs, tf.float16)
      outputs = outputs * scale_inputs * scale_w + offset_inputs * self.kernel * scale_w
      p =tf.Print([scale_inputs, scale_w, offset_inputs],[scale_inputs, scale_w, offset_inputs])
   # p1 = tf.Print([shift_inputs, f_inputs, shift_w, f_w],[shift_inputs, f_inputs, shift_w, f_w])
   ...
   #   outputs = tf.identity(outputs)
    ...
    #  outputs,_,_,_ =fixed(outputs,0,8,2)
...
class SeparableConv1D(SeparableConv):
  ...
  def call(self, inputs):
    ...
    outputs = nn.separable_conv2d(#liuchang
        inputs,
        depthwise_kernel,
        pointwise_kernel,
        strides=strides,
...
class SeparableConv2D(SeparableConv):
  ...
  def call(self, inputs):
    ...
    if self.data_format == 'channels_last':
     ...
     # self.depthwise_kernel,_,_,_ = fixed(self.depthwise_kernel,8,2)
     # self.pointwise_kernel,_,_,_ = fixed(self.pointwise_kernel,8,2)
