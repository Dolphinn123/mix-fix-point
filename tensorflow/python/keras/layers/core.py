...
class Dense(Layer):
  ...
  #  self.kernel,_ = fixed(self.kernel,bitnum=8,interval=3,steps_per_epoch=1250)
    # fintune 
   ...
   # self.kernel, shift_w, f_w = fixed_inf(self.kernel, 0, bitnum=8, mode=1, isfp16=isfp16)
    ...
    if isfp16 is True:
      p4 = tf.Print([self.kernel],[self.kernel])
      p3 = tf.Print([outputs],[outputs])
      ...
      p2 = tf.Print([inputs],[inputs])
      with tf.control_dependencies([p1,p,p2,p4,p3]):
